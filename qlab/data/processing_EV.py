...
Shows % of up bars (Close > Open) across intraday timeframes for previous trading day.
...
HEATMAP_MAX_TICKERS  = 120
TOP_PER_GROUP        = 15               # ← NEW: max stocks shown per "best timeframe" group

INTERVALS = ['1min', '2min', '3min', '5min', '7min', '15min', '30min', '60min', '120min', '240min']  # ← changed 'T' to 'min'
...
def analyze_day(df_1min: pd.DataFrame, target_date: datetime.date) -> dict | None:
    ...
    for intv, label in zip(INTERVALS, INTERVAL_LABELS):
        ...
        total = len(resampled)
        # Optional: filter low-bar counts to avoid 100% on sparse data (uncomment if desired)
        # if total < 5:  # e.g., require at least 5 bars per interval
        #     results[label] = 0.0
        #     continue
...
def main():
    ...
    print(f"\nAnalyzing previous trading day: {prev_day}\n")
    ...
    with tqdm(total=len(all_symbols), desc="Processing tickers", unit="ticker",
        ...
        for symbol in all_symbols:
            ...
            day_stats = analyze_day(df, prev_day)
            ...
            processed = pbar.n + 1  # current one is finished
    ...
    if df_filtered.empty:
        print(f"No stocks reached ≥{MIN_THRESHOLD}% up bars on any timeframe.")
        return
    ...
    if SAVE_INTERMEDIATE_CSV:
        csv_path = f"{CSV_OUTPUT_SUFFIX}_{prev_day}.csv"
    ...
    for tf, group in grouped:
        ...
        # ── Create separate heatmap for this group ──────────────────────────
        ...
        if not heatmap_data.empty:
            ...
            plt.title(f"{tf} – Top performers {prev_day}\n(filtered ≥{MIN_THRESHOLD}%)",
                      fontsize=13, pad=12)
            plt.ylabel("Ticker")
            ...
            # Unique filename per group
            safe_tf = tf.replace(" ", "").replace(":", "")
            heatmap_file = f"heatmap_{prev_day}_{safe_tf}_top{len(heatmap_data)}.png"
            plt.savefig(heatmap_file, dpi=140, bbox_inches='tight')
            plt.close()  # free memory
            print(f"  → Heatmap saved: {heatmap_file}")

    print("\nAll group heatmaps generated.")
    ...
    if not heatmap_data_all.empty:
        ...
        plt.title(f"Overall Top Intraday % Up Bars – {prev_day}\n(top {len(heatmap_data_all)} shown)",
                  fontsize=14, pad=16)
        plt.ylabel("Ticker")
        plt.xlabel("Timeframe")
        plt.xticks(rotation=45, ha='right')
        ...
        overall_file = f"heatmap_{prev_day}_overall_top{len(heatmap_data_all)}.png"
